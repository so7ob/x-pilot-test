...
- الاستثناء المرئي الوحيد: نافذة إعداد تسجيل الدخول، تفتح بطلب صريح من المستخدم فقط. منذ v1.5.4 تفتح على **Chrome المثبّت لديك** (على دليل بيانات Runner الخاص، وليس ملفك اليومي) وبلا علامات أتمتة — فيعمل نموذج دخول X المتدرج وزر «المتابعة باستخدام Google» (كان العطلان: توقف صامت بعد اسم المستخدم، ورفض Google «المتصفح غير آمن» — المشكلة رقم 12).
...
- منذ v1.5.6 صارت فحوص الجاهزية (Preflight/Dry Run/التشخيص) محصّنة ضد سباقات جاهزية التبويب: التبويب المؤقت يُفتح مباشرة على رابط X مع بوابة URL على انتظار التحميل وفحص X مستقر بإعادة محاولة محدودة (المشكلة رقم 18 — كانت تظهر كـ«X Adapter لم يتعرف على الصفحة» بلا سبب)، وأي فشل الآن يعرض سببه المترجم تحت الفحص.
...
- محرك «تبويب Chrome» كان معطلًا بالكامل في v1.5.0–v1.5.4 (سكربت محتوى ES Module — المشكلة رقم 15، أُصلح في v1.5.5). إن ظهر «Could not establish connection. Receiving end does not exist.» فأعد تحميل الإضافة من chrome://extensions وأعد المحاولة. وحتى v1.5.5 كانت فحوص الجاهزية عرضة لسباق جاهزية التبويب (X_INSPECT قبل التزام تنقل X/تسجيل السكربت — المشكلة رقم 18، أُصلحت في v1.5.6).
